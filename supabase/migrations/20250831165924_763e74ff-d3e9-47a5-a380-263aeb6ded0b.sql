-- Update the sync_lead_assignments function to NOT delete multiple assignments
-- This allows a lead to be assigned to multiple partners
CREATE OR REPLACE FUNCTION public.sync_lead_assignments()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  -- For each lead that has assignments, update quiz_responses with the latest assignment
  FOR lead_record IN 
    SELECT DISTINCT quiz_response_id 
    FROM lead_assignments 
  LOOP
    -- Get the latest assignment for this lead (for the assigned_partner_id field)
    ...
    -- Update quiz_responses with the latest assignment for compatibility
    UPDATE quiz_responses 
    SET assigned_partner_id = latest_assignment.partner_id,
        updated_at = now()
    WHERE id = lead_record.quiz_response_id;
    
    -- REMOVED: Don't delete other assignments - allow multiple partners per lead
  ...
  WHERE qr.assigned_partner_id IS NOT NULL
    AND NOT EXISTS (
      ...
      WHERE la.quiz_response_id = qr.id
        AND la.partner_id = qr.assigned_partner_id
    );
