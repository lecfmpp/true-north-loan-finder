...
-- Create payment reminder email template using existing sequence type
INSERT INTO public.email_sequences (name, description, sequence_type, is_active)
VALUES ('payment_reminder', 'Payment reminder for lead simulation submissions', 'admin_notification', true);
...
DECLARE
    seq_id UUID;
BEGIN
    SELECT id INTO seq_id FROM public.email_sequences WHERE name = 'payment_reminder';
