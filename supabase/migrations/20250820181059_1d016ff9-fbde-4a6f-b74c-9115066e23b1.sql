-- Update get_roi_metrics function to use complete qualified lead criteria
CREATE OR REPLACE FUNCTION public.get_roi_metrics(start_date date DEFAULT (CURRENT_DATE - '30 days'::interval), end_date date DEFAULT CURRENT_DATE)
 RETURNS TABLE(total_leads bigint, total_spend integer, cost_per_lead numeric, total_revenue integer, roi_percentage numeric, channel_breakdown jsonb, qualified_leads bigint, funded_leads bigint, all_leads bigint, application_leads bigint, commission_generated numeric)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Get qualified leads with complete criteria (matching Report Dashboard logic)
  SELECT COUNT(*) INTO qualified_count
  FROM public.quiz_responses 
  WHERE DATE(created_at) BETWEEN start_date AND end_date
  AND monthly_revenue >= 10000
  AND time_in_business IN ('6-12', '1-2', '2-5', '5+', '+5')
  AND (
    CASE credit_score
      WHEN 'excellent' THEN 775
      WHEN 'good' THEN 725
      WHEN 'fair' THEN 675
      WHEN 'poor' THEN 625
      WHEN 'unsure' THEN 650
      ELSE COALESCE(NULLIF(regexp_replace(credit_score, '[^0-9]', '', 'g'), '')::integer, 0)
    END
  ) >= 600;
