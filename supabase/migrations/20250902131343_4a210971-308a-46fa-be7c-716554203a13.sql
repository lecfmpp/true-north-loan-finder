...
-- Add validation triggers to ensure email addresses always match authenticated user
...
-- Add trigger to log sensitive data access (for compliance)
...
-- Add audit triggers for SELECT operations on sensitive tables
CREATE TRIGGER audit_usa_application_access
    AFTER SELECT ON public.usa_applications
    FOR EACH ROW EXECUTE FUNCTION public.check_sensitive_data_access();

CREATE TRIGGER audit_usa_simplified_application_access
    AFTER SELECT ON public.usa_applications_simplified  
    FOR EACH ROW EXECUTE FUNCTION public.check_sensitive_data_access();

CREATE TRIGGER audit_canadian_application_access
    AFTER SELECT ON public.canadian_applications
    FOR EACH ROW EXECUTE FUNCTION public.check_sensitive_data_access();
...
COMMENT ON FUNCTION public.check_sensitive_data_access() IS 'Logs access to sensitive application data for compliance and security monitoring';