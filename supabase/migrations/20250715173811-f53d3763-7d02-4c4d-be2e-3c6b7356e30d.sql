-- First update any existing 'admin' roles to 'superadmin'
...
ALTER TABLE public.profiles ALTER COLUMN role TYPE user_role USING 
  CASE 
    ...
    WHEN role = 'lender' THEN 'lender'::user_role
    WHEN role = 'broker' THEN 'broker'::user_role
    ELSE 'user'::user_role
  END;
...
-- Ensure the superadmin user is properly set
UPDATE public.profiles 
SET role = 'superadmin' 
WHERE user_id IN (
