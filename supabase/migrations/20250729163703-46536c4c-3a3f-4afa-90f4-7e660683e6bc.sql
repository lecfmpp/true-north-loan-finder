-- Step 1: Fix Critical Privilege Escalation - Create new role system without breaking existing
-- Create user roles enum (using same values as existing user_role)
...
-- Create NEW security definer functions (prevents RLS recursion)
...
-- Migrate existing roles from profiles table (fixed casting)
INSERT INTO public.user_roles (user_id, role, assigned_at)
SELECT user_id, 
  CASE 
    WHEN role::text = 'superadmin' THEN 'superadmin'::app_role
    WHEN role::text = 'lender' THEN 'lender'::app_role
    WHEN role::text = 'broker' THEN 'broker'::app_role
    ELSE 'user'::app_role
  END,
  created_at
...
-- Update existing database functions to use new role system (replace content, not drop)
