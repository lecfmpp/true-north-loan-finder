...
-- This migration adds enhanced audit logging and strengthens RLS policies
...
-- Drop and recreate Canadian applications policies with stricter controls
DROP POLICY IF EXISTS "Users can view only their own Canadian applications" ON canadian_applications;
CREATE POLICY "Users can view only their own Canadian applications"
  ON canadian_applications
  FOR SELECT
  USING (
    auth.uid() = user_id 
...
DROP POLICY IF EXISTS "Secure Canadian application creation" ON canadian_applications;
CREATE POLICY "Secure Canadian application creation"
  ON canadian_applications
  FOR INSERT
  WITH CHECK (
...
-- Drop and recreate USA applications policies with stricter controls
DROP POLICY IF EXISTS "Users can view only their own USA applications" ON usa_applications;
CREATE POLICY "Users can view only their own USA applications"
  ON usa_applications
  FOR SELECT
  USING (
    auth.uid() = user_id 
...
DROP POLICY IF EXISTS "Secure USA application creation" ON usa_applications;
CREATE POLICY "Secure USA application creation"
  ON usa_applications
  FOR INSERT
  WITH CHECK (
    auth.uid() IS NOT NULL 
