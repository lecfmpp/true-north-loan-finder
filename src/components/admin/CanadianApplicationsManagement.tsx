...
const CanadianApplicationsManagement: React.FC<CanadianApplicationsManagementProps> = ({ onCountUpdate, restrictToQuizIds, partnerMode }) => {
  ...
  const [filteredDrafts, setFilteredDrafts] = useState<CanadianDraft[]>([]);
  const [selectedDraft, setSelectedDraft] = useState<CanadianDraft | null>(null);
  ...
  const fetchApplications = async () => {
    try {
      ...
      // Fetch Canadian application drafts
  ...
  const [draftToDelete, setDraftToDelete] = useState<CanadianDraft | null>(null);
  const [draftDeleteConfirmText, setDraftDeleteConfirmText] = useState("");

  const openDraftDeleteModal = (draft: CanadianDraft) => {
      ...
      {filteredDrafts.length > 0 && (
        <div className="grid gap-4">
          {filteredDrafts.map((draft) => {
            ...
            const company = fd.legal_business_name || fd.legal_corporation_name || 'Untitled';
            const email = fd.email_address || '—';
            const phone = fd.business_phone || fd.telephone_number || '—';
            return (
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <p className="font-medium">{fd.legal_business_name || fd.legal_corporation_name || 'N/A'}</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p>{fd.business_phone || fd.telephone_number || 'N/A'}</p>
                    </div>
                    <div>
                      <span className="text-sm font-medium text-muted-foreground">Loan Amount</span>
                      <p>{fd.amount_requested ? `$${Number(fd.amount_requested).toLocaleString()}` : 'N/A'}</p>
                    </div>
